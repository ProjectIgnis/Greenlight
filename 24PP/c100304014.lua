...
function s.cfilter(c,e,tp)
	...
	return og:IsExists(Card.IsAbleToRemove,2,nil)
		and Duel.IsExistingMatchingCard(s.ssfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c:GetRank()+1)
end
function s.ssfilter(c,e,tp,rank)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Remove(rg,nil,REASON_EFFECT)==2 and Duel.IsExistingMatchingCard(s.ssfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,rank) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sc=Duel.SelectMatchingCard(tp,s.ssfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,rank):GetFirst()
