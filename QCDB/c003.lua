...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetMZoneCount(tp)>=2
		...
		and Duel.IsExistingTarget(s.spfilter,tp,LOCATION_GRAVE,0,1,nil,tp) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)>0 and tc:IsRelateToEffect(e) then
		...
		if #synchg>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
			...
			if sc then
				Duel.BreakEffect()
				Duel.SynchroSummon(tp,sc,nil,c)
			end
		end
	end
end
function s.efcon(e,tp,eg,ep,ev,re,r,rp)
	return r&REASON_SYNCHRO>0 and e:GetHandler():GetReasonCard():IsRace(RACE_DRAGON)
end
