...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Keep track of the monsters Special Summoned by the opponent
...
function s.cfilter(c,tp,e)
	return c:IsSummonPlayer(1-tp) and c:IsLocation(LOCATION_MZONE) and (not e or c:IsCanBeEffectTarget(e))
end
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local g=s.sumgroup:Filter(s.cfilter,nil,tp,e)
	if chkc then return g:IsContains(chkc) and s.cfilter(chkc,tp,e) end
