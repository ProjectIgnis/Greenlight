...
s.listed_series={SET_DIABELLESTAR}
function s.plfilter(c,tp,hand_chk)
	...
	return c:IsMonster() and c:IsSetCard(SET_DIABELLESTAR) and ft>0
		and c:CheckUniqueOnField(owner) and not c:IsForbidden()
end
function s.pltg(e,tp,eg,ep,ev,re,r,rp,chk)
	local hand_chk=e:GetHandler():IsLocation(LOCATION_HAND)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(s.plfilter,tp,LOCATION_HAND|LOCATION_DECK|LOCATION_GRAVE,0,1,nil,tp,hand_chk) end
end
function s.plop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.plfilter),tp,LOCATION_HAND|LOCATION_DECK|LOCATION_GRAVE,0,nil)
	if #g>0 then
