...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Destroy(g,REASON_EFFECT)~=0 and c:IsRelateToEffect(e) then
		local og=Duel.GetOperatedGroup():Filter(Card.IsMonster,nil)
		local mg=c:GetOverlayGroup():Filter(s.matfilter,nil)
		if mg:GetClassCount(Card.GetCode)>=3 and #og>0 then
			Duel.BreakEffect()
			Duel.Damage(1-tp,og:GetSum(Card.GetBaseAttack),REASON_EFFECT)
		end
	end
end
...
function s.copytg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SelectTarget(tp,s.copyfilter,tp,0,LOCATION_GRAVE,1,1,nil)
end
